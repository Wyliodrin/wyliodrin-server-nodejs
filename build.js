...
{
  var makeProcess = child_process.spawn(command,args,
    {cwd:path, env:_.extend(process.env,{wyliodrinid:id, wyliodrinport:port})});
      ...
      child_process.exec('rm -rf '+buildPath, {maxBuffer:10*1024, cwd:buildFile},
        function(error, stdout, stderr)
        {
          if(files.canMount())
          {
            ...
            child_process.exec('cp -rfv '+mountPath+'/'+id+' '+buildFile+' && chmod -R u+w '+buildFile, 
              {maxBuffer: 30*1024, cwd:buildFile}, 
              function(error, stdout, stderr)
              {
                if (!error)
                { 
                  console.log ('ln -s Makefile.'+gadget+' Makefile '+buildPath+'/'+id);
                  child_process.exec('ln -s Makefile.'+gadget+' Makefile', {cwd: buildPath}, 
          ...
          else
          {
            ...
            child_process.exec('wget --no-check-certificate '+address, {maxBuffer:30*1024, cwd:buildFile},
              function(error,stdout,stderr)
              {
                if(!error)
                {
