...
xmpp.Client.prototype.load = function (wother)
{
	...
	this.on ('stanza', function (stanza)
	{
		...
		if (stanza.is('message'))
		{
			_.each (stanza.children, function (es)
			{
				...
				if (es.getNS() == WYLIODRIN_NAMESPACE)
				{
					...
					if (t.tags().has(name)) t.tags().get(name)(t, xmpp.JID(stanza.attrs.from).bare().toString().toLowerCase(), xmpp.JID(stanza.attrs.to).bare().toLowerCase(), es, error);
					else if (other) other (t, xmpp.JID (stanza.attrs.from).bare().toString().toLowerCase(), xmpp.JID(stanza.attrs.to).bare().toLowerCase(), es, error);
				}
			});
		}
		else
		if (stanza.is('presence'))
		{
			...
			var error = stanza.attrs.type == 'error';
			if (t.tags().has(name)) t.tags().get(name)(t, xmpp.JID(stanza.attrs.from).bare().toString().toLowerCase(), xmpp.JID(stanza.attrs.to).bare().toLowerCase(), stanza, error);
			else if (other) other (t, xmpp.JID(stanza.attrs.from).bare().toString().toLowerCase(), xmpp.JID(stanza.attrs.to).bare().toLowerCase(), stanza, error);
		}
	});
}
